...
export default function AgendaScreen() {
  ...
  async function ajouterEvent() {
    if (!titre.trim() || !date.trim()) { Alert.alert('Erreur', 'Titre et date sont obligatoires'); return; }
    setAdding(true);
    try {
      const { data: { user } } = await supabase.auth.getUser();
      ...
      const { data: newEvent, error } = await supabase.from('nursery_events').insert({
        creche_id: prof.creche_id, titre: titre.trim(), type: type || 'Autre',
        date: dateNorm, description: description.trim() || null
      }).select().single();
      ...
      >
        {events.length === 0 ? (
          <View style={s.empty}>
            ...
            {isDirectrice && <Text style={s.emptySubtext}>Publiez des événements pour informer les parents</Text>}
          </View>
        ) : (
          <View style={s.list}>
            {events.map(event => {
                ...
                <View key={event.id} style={s.eventCard}>
                  <View style={s.eventLeft}>
                    <View style={s.eventDateBox}>
                      ...
                      <Text style={s.eventMonth}>{d ? d.toLocaleDateString('fr-FR', { month: 'short' }) : '?'}</Text>
                    </View>
                  </View>
  ...
  <Text style={s.deleteBtnIcon}>🗑️</Text>
      ...
      >
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
          {EVENT_TYPES.map(tp => (
            <TouchableOpacity key={tp} style={[s.typeChip, type === tp && s.typeChipActive]} onPress={() => setType(tp)}>
...
const styles = (theme) => StyleSheet.create({
  ...
  list: { padding: 16 },
  eventCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: theme.card, borderRadius: 16, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: theme.border },
  eventLeft: { marginRight: 14 },
  eventDateBox: { backgroundColor: theme.primaryLight, borderRadius: 10, padding: 8, alignItems: 'center', minWidth: 44 },
